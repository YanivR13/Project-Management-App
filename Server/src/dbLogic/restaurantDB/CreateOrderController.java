...
public class CreateOrderController { // Start of CreateOrderController class definition
    ...
    // Static formatter to ensure SQL DATETIME strings follow the correct 'YYYY-MM-DD HH:mm:00' format
    private static final DateTimeFormatter sqlFormatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:00"); // Formatter definition
    ...
    public static ServiceResponse processNewReservation(Reservation res) { // Start of processNewReservation method
        ...
        try { // Start of main try block to catch logic or parsing errors
            ...
            LocalDateTime requestedDT = LocalDateTime.parse(res.getReservationDateTime(), sqlFormatter); // Parsing operation
            ...
            // Safety Check: Ensure the restaurant data is actually loaded in the server memory
            if (restaurant == null) { // Check if restaurant instance is null
                ...
                return new ServiceResponse(ServiceStatus.INTERNAL_ERROR, "Server Error: Restaurant data not initialized."); // Error return
            } // End of null check
            ...
            // Logic: The restaurant must be open at the start AND at the end of the 2-hour window
            ...
            // Search for an available table that fits the party size for the exact requested slot
            int allocatedTableSize = findAvailableTableSize(restaurant, requestedDT, res.getNumberOfGuests()); // Calling table search

            if (allocatedTableSize != -1) { // If a table was successfully found (allocatedTableSize is not -1)
                
                // Persist the reservation in the database and get the generated confirmation code
                ...
                return new ServiceResponse(ServiceStatus.INTERNAL_ERROR, "Database failed to save the reservation."); // Save error return
            } // End of Phase 1 success check

            // --- PHASE 2: 3-Day Lookahead (Intelligent Suggestions) ---
            
            // If the requested slot is full, iterate through the next 3 days to find a match
            for (int i = 1; i <= 3; i++) { // Start of lookahead loop
                ...
                // Check 1: Is the restaurant open for the full duration on this alternative day?
                boolean isNextDayOpen = (restaurant.isOpen(nextDate, nextStartTimeStr) && restaurant.isOpen(nextDate, nextEndTimeStr)); // Boolean check
                
                if (isNextDayOpen) { // If open on the alternative day
                    
                    // Check 2: Is there an available table of the required size at this suggested time?
                    int suggestedTableSize = findAvailableTableSize(restaurant, nextDT, res.getNumberOfGuests()); // Checking availability
                    ...
                    if (suggestedTableSize != -1) { // If capacity is found on this alternative day
                        ...
                        String suggestion = nextDT.format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm")); // Formatting suggestion
                        
                        // Return a suggestion response containing the alternative slot
                        return new ServiceResponse(ServiceStatus.RESERVATION_SUGGESTION, suggestion); // Suggestion return
            ...
            // If no slots were found in the requested time or the 3-day window, inform the client
            return new ServiceResponse(ServiceStatus.RESERVATION_FULL, "No tables available for the next 3 days."); // Full return
            ...
            return new ServiceResponse(ServiceStatus.INTERNAL_ERROR, "Server Exception: " + e.getMessage()); // Exception return
    ...
    private static int findAvailableTableSize(Restaurant restaurant, LocalDateTime dt, int requestedSize) { // Start method
        ...
        for (int capacity : sortedCapacities) { // Iterate through each available table size
            ...
            // Calculate how many tables of this specific size are already booked at this time
    ...
    private static int getReservedTablesCount(LocalDateTime dt, int capacity) { // Start method
        ...
        try (PreparedStatement pstmt = DBController.getInstance().getConnection().prepareStatement(query)) { // Prepare statement
            ...
            // Bind the requested time to the window boundaries (using DATE_SUB and DATE_ADD in SQL)
        ...
        // Safety Policy: If database fails, return a high number to prevent accidental overbooking
    ...
    private static Long saveNewReservation(Reservation res, int finalTableSize) { // Start method
        ...
        String query = "INSERT INTO reservation (reservation_datetime, number_of_guests, user_id, status) VALUES (?, ?, ?, ?)"; // SQL string
        
        // Obtain connection from the DB singleton
        Connection conn = DBController.getInstance().getConnection(); // Getting connection
        
        try (PreparedStatement pstmt = conn.prepareStatement(query, Statement.RETURN_GENERATED_KEYS)) { // Prepare with key retrieval
